...
export function calculateQuestProgress(
  ...
  const completedTasksToday = tasks.filter(task => {
    ...
    // If no timestamp but completed is true, only count if it was completed today
    // For safety, if there's no timestamp, don't count it (to prevent false positives)
    // This ensures we only count tasks that were actually completed today
    return false;
  });
  
  const highImpactTasksToday = completedTasksToday.filter(task => task.impact === 'high');
  
  // Calculate XP earned TODAY only (from tasks completed today)
  const xpEarnedToday = completedTasksToday.reduce((total, task) => {
    return total + (task.points || 0);
  }, 0);
  
  return {
    tasksCompleted: completedTasksToday.length,
    xpEarned: xpEarnedToday, // Only today's XP, not cumulative
    streakCount: streak,
    highImpactTasks: highImpactTasksToday.length,
    lastResetDate: today
  };
}
