...
interface Task {
  ...
  completedAt?: string; // ISO timestamp when task was completed (for accurate daily quest tracking)
}
...
export default function HypeOSPage() {
      ...
      try {
        ...
        if (hasGoalData) {
          ...
          if (userToLoad) {
            ...
            // Initialize or load quests - always recalculate progress based on today's tasks
            const initialQuests = initializeQuests();
            if (sessionData?.hypeos?.quests && sessionData.hypeos.quests.length > 0) {
              // Use saved quest structure but recalculate progress from today's tasks
              const savedQuests = sessionData.hypeos.quests;
              const currentTasks = tasks.length > 0 ? tasks : (sessionData?.hypeos?.tasks || []);
              const currentUser = userWithOnboarding;
              
              // Recalculate quest progress based on TODAY's tasks only (very accurate)
              const accurateProgress = calculateQuestProgress(
                currentTasks,
                currentUser.hypePoints || 0,
                currentUser.currentStreak || 0
              );
              const recalculatedQuests = updateQuestProgress(initialQuests, accurateProgress);
              setQuests(recalculatedQuests);
              console.log('✅ Loaded and recalculated quests based on today\'s tasks only:', {
                tasksCompletedToday: accurateProgress.tasksCompleted,
                xpEarnedToday: accurateProgress.xpEarned
              });
            } else {
              // No saved quests - use initialized quests
              setQuests(initialQuests);
            }
  ...
  // Recalculate quest progress whenever tasks change (to ensure accuracy)
  useEffect(() => {
    if (!user || isLoading || tasks.length === 0 || quests.length === 0) return;
    
    // Recalculate quest progress based on TODAY's tasks only (very accurate)
    const progress = calculateQuestProgress(tasks, user.hypePoints || 0, user.currentStreak || 0);
    const updatedQuests = updateQuestProgress(quests, progress);
    
    // Only update if there's a meaningful change (to prevent loops)
    const hasChange = updatedQuests.some((q, i) => 
      q.current !== quests[i]?.current || q.completed !== quests[i]?.completed
    );
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tasks, user?.hypePoints, user?.currentStreak]); // quests intentionally excluded to prevent loops
  ...
  const completeTask = (taskId: number) => {
    ...
    // Update task completion with timestamp (for accurate daily quest tracking)
    const todayISO = new Date().toISOString();
    const updatedTasks = tasks.map(t => 
      t.id === taskId ? { ...t, completed: true, completedAt: todayISO } : t
    );
    ...
    // Reuse todayISO from above (already defined at line 2139)
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-2 border-gray-200 dark:border-gray-800 border-t-[#39d2c0] mx-auto mb-4"></div>
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 pt-28">
          ...
          {user && (
            <div className="flex items-center gap-8 pt-6 border-t border-gray-200 dark:border-gray-800">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-semibold text-gray-900 dark:text-white">{user?.currentStreak || 0} days</span>
              </div>
              <div className="flex items-center gap-2">
                <Star className="w-4 h-4 text-purple-500" />
                <span className="text-sm text-gray-600 dark:text-gray-400">Level</span>
                <span className="text-base font-semibold text-gray-900 dark:text-white">{user?.level || 1}</span>
              </div>
              <div className="flex items-center gap-2">
                <Zap className="w-4 h-4 text-[#39d2c0]" />
                <span className="text-sm text-gray-600 dark:text-gray-400">Points</span>
                <span className="text-base font-semibold text-gray-900 dark:text-white">{(user?.hypePoints || 0).toLocaleString()}</span>
              </div>
        ...
        {user && viewMode === 'default' ? (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
              ...
              <div className="space-y-8">
                {/* Today's Tasks */}
                <div className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-gray-800">
                  <DailyFocusCard
                    tasks={(() => {
                    ...
                    onTaskSkip={(taskId) => console.log('Task skipped:', taskId)}
                ...
                {reviewQueue && reviewQueue.totalItems > 0 && (
                  <div className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-gray-800">
                    <ReviewQueueCard
                      queue={reviewQueue}
                      onTaskClick={(taskId) => console.log('Review task clicked:', taskId)}
                      onStartReview={() => console.log('Starting review session')}
                    />
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-gray-800 p-8">
              <h2 className="text-xl font-medium text-gray-900 dark:text-white mb-6">All Goals</h2>
              <div>
                {allGoals.length === 0 ? (
                  <div className="text-center py-16">
                    <div className="w-12 h-12 rounded-full bg-[#f5f5f5] dark:bg-[#2e2e2e] flex items-center justify-center mx-auto mb-4">
                      <Target className="h-6 w-6 text-[#787878] dark:text-[#9b9b9b]" />
                    </div>
                    <p className="text-[15px] text-[#787878] dark:text-[#9b9b9b] mb-6">
                      You haven't created any goals yet.
                    </p>
                    <Button 
                      variant="ghost"
                      className="text-[13px] text-[#787878] dark:text-[#9b9b9b] hover:text-[#2e2e2e] dark:hover:text-[#ededed]"
                      onClick={() => setShowOnboarding(true)}
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {allGoals.map((goal, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className={`cursor-pointer transition-all duration-150 rounded-lg border p-6 hover:border-[#39d2c0] hover:shadow-sm ${
                            isActive 
                        ...
                        >
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex-1">
                              ...
                              <span className="inline-block text-xs text-gray-600 dark:text-gray-400 px-2 py-1 rounded bg-gray-100 dark:bg-gray-800">
                                {goal.category || 'General'}
                              </span>
                            </div>
                            {isActive && (
                              <div className="w-2 h-2 rounded-full bg-[#39d2c0]"></div>
                            )}
                          </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="w-full h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                              <div 
                                ...
                                style={{ width: `${goal.goalProgress || 0}%` }}
                              />
                            </div>
          ...
          <div className="mt-16 pt-8 border-t border-gray-200 dark:border-gray-800">
            <div className="flex flex-wrap gap-3 justify-center">
              <button
                ...
                className="px-6 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors inline-flex items-center space-x-2"
              >
      ...
      {user && (
        <MoreTasksModal
          ...
          userGoal={user.goalTitle || ''}
          userCategory={user.category || 'general'}
          userTarget={user.goalTarget || ''}
          userCurrent={user.goalCurrent || ''}
          existingTasks={tasks}
        />
      )}
    </div>
